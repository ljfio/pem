...
module.exports.createPkcs8 = createPkcs8
...
* @param {Object} [options] object of cipher and optional client key password {cipher:'aes128', password: 'xxxx'}
* @param {Function} callback Callback function with an error object and {key}
*/
function createPkcs8 (key, options, callback) {
  if (!callback && typeof options === 'function') {
    callback = options
    options = {}
  }

  var params = ['pkcs8', '-topk8']
  var delTempPWFiles = []
  ...
  if (options.cipher && options.password) {
    ...
    helper.createPasswordFile({'cipher': options.cipher, 'password': options.password, 'passType': 'in'}, params, delTempPWFiles[delTempPWFiles.length])
    // create password file for the "-passout"
    helper.createPasswordFile({'cipher': options.cipher, 'password': options.password, 'passType': 'out'}, params, delTempPWFiles[delTempPWFiles.length])
  } else {
    params.push('-nocrypt')
  }
  ...
  openssl.exec(params, 'PRIVATE KEY', tmpfiles, function (sslErr, data) {
    function done (err) {
      if (err) {
        return callback(err)
      }
